...
def main(q):
    ...
    cuda_secondary=os.environ.get('DREAMBOOTH_SECONDARY', accelerator.device)
    ...
    text_encoder.to(cuda_secondary)
    vae.to(cuda_secondary)
    ...
    q.put(len(train_dataloader))
    for epoch in range(args.num_train_epochs):
        
        for step, batch in enumerate(train_dataloader):
                # Convert images to latent space
            with torch.no_grad():
                    latents = vae.encode(batch["pixel_values"].to(cuda_secondary)).latent_dist.sample()
                    latents = latents.to(cuda_secondary) * 0.18215

                # Get the text embedding for conditioning
            with torch.no_grad():
                    encoder_hidden_states = text_encoder(batch["input_ids"].to(cuda_secondary))[0]

            q.put((latents.to("cpu"),encoder_hidden_states.to("cpu")))
